...
import 'package:mynotes/presentation/bloc/params/unified_items_params.dart';
...
class UnifiedItemsBloc extends Bloc<UnifiedItemsEvent, UnifiedItemsState> {
  UnifiedItemsBloc()
    : super(const UnifiedItemsState(params: UnifiedItemsParams())) {
  ...
  void _onChangeFilter(
    ...
    Emitter<UnifiedItemsState> emit,
  ) {
    emit(
      UnifiedItemsState(
        params: state.params.copyWith(selectedFilter: event.filter),
      ),
    );
  }
  ...
  void _onChangeSort(ChangeSortEvent event, Emitter<UnifiedItemsState> emit) {
    emit(
      UnifiedItemsState(params: state.params.copyWith(sortBy: event.sortBy)),
    );
  }

  void _onTogglePinned(
    TogglePinnedFilterEvent event,
    Emitter<UnifiedItemsState> emit,
  ) {
    emit(
      UnifiedItemsState(
        params: state.params.copyWith(
          showOnlyPinned: !state.params.showOnlyPinned,
        ),
      ),
    );
  }
  ...
  void _onUpdateSearch(
    ...
    final smartFilter = NLFilterParser.parse(event.query);
    emit(
      UnifiedItemsState(
        params: state.params.copyWith(
